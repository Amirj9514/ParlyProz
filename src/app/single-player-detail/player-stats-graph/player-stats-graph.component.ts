...
export class PlayerStatsGraphComponent {
  ...
  //             .text('0')
  //             .attr('x', x + xScale.bandwidth() / 2)
  //             .attr('y', height - smallBarHeight / 2)
  //             .attr('text-anchor', 'middle')
  //             .attr('fill', 'black')
  //             .attr('font-size', '12px')
  //             .attr('font-weight', 'bold');
  //           return;
  //         }
  //         const visibleStacks = allKeys.filter((k) => d.values[k] > 0).length;
  ...
  //           const totalValue = d3.sum(allKeys, (k) => d.values[k]);
  //           const barColor: string =
  //             totalValue >= lineValue
  //               ? (colorAbove(key) as string)
  //               : (colorBelow(key) as string);

  //           const lastVisibleStack = allKeys
  //             .slice() // Clone array
  //             .reverse() // Reverse to find last non-zero value
  //             .find((k) => d.values[k] > 0);

  //           const isTopStack = key === lastVisibleStack;

  //           const pathD = isTopStack
  ...
  //           .attr('font-size', '12px')
  //           .attr('font-weight', 'bold')
  //           .style('display', visibleStacks === 1 ? 'none' : 'block');
  //       });
